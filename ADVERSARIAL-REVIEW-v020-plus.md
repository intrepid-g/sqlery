...
> **Pushback + partial resolution 2026-05-25 (v0.22.0).** The rebuttal above was Django-only and overstated cross-mode coverage. Two of the five heuristics did not exist in standalone mode: `refresh_worker_heartbeat()` was missing from the SQLAlchemy backend, and the zombie detector (`daemon._fail_zombie_running_jobs`) queried Django `QueuedJob.objects` directly and short-circuited with `if QueuedJob is None: return` — so standalone daemons got **no** heartbeat refresh and **no** zombie recovery. v0.22.0 closes this: heartbeat parity added to the SQLAlchemy backend, and the five-heuristic logic extracted into mode-agnostic core (`core/liveness.py` + the `get_running_jobs_for_liveness` / `fail_zombie_job` backend methods). Django behavior unchanged. **Residual caveat:** the daemon bumps the heartbeat on **PID-liveness alone** after sending SIGUSR1 (`daemon.py` ~line 857), so a locally-hung-but-alive worker keeps a fresh heartbeat — local heartbeat-staleness (Check 5) is therefore a weak signal, and zombie recovery for a wedged local worker leans on Check 4 (worker moved on / idle), not staleness. "Liveness is DB-backed by design" holds for *PID* liveness, not *execution* liveness.
