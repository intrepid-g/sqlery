...
from sqlery.worker_pool import get_worker_pool_status, stop_all_workers
from sqlery.worker_registry import kill_worker, cleanup_dead_workers
...
class Command(BaseCommand):
    ...
    def kill_worker(self, worker_id, force=False):
        ...
        try:
            if kill_worker(worker_id, force=force):
                sig = "SIGKILL" if force else "SIGTERM"
                self.stdout.write(
                    self.style.SUCCESS(f"✓ Worker killed ({sig})")
                )
            else:
                self.stdout.write(
                    self.style.WARNING("⚠ Worker not found or already dead")
                )
        except Exception as e:
            self.stderr.write(
    ...
    def stop_all_workers(self, force=False):
        ...
        try:
            count = stop_all_workers(force=force)
            if count > 0:
                sig = "SIGKILL" if force else "SIGTERM"
                self.stdout.write(
