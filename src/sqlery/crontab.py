"""Cron expression parsing - vendored from crontabula.

Original: https://github.com/orf/crontabula
License: MIT
Credits: Tom Forbes (https://github.com/orf)

Vendored to eliminate external dependencies for sqlery MVP.
"""

import dataclasses
# from typing import List, Iterator, Optional  # Replaced with modern syntax (Python 3.10+)
from collections.abc import Iterator
import datetime
import calendar

__all__ = ["Crontab", "parse_cron_string", "InvalidExpression", "next_cron_occurrence"]
...
class Crontab:
    ...
    def next_from(self, start: datetime.datetime | None = None) -> datetime.datetime:
        """
        Return the next time at which this crontab should execute

        >>> crontab = parse_cron_string("*/10 * * * *")
        >>> crontab.next_from()
        datetime.datetime(...)
        """
        return next(self.date_times(start))
    ...
    def date_times(
        ...
        """
        anchor = start if start else datetime.datetime.now(datetime.timezone.utc)
    ...
    def dates(self, start: datetime.date | None = None) -> Iterator[datetime.date]:
        """
        ...
        while True:
            for month in self.months:
                ...
                for day_of_month, day_of_week in cal.itermonthdays2(anchor.year, month):
                    ...
                    day_of_week_match = _day_of_week_to_cron(day_of_week) in self.day_of_week
...
def next_cron_occurrence(
    cron_string: str, base_date: datetime.datetime | None = None
) -> datetime.datetime:
    """
    Calculates the next occurrence of a cron schedule based on the given cron string.

    Args:
        cron_string (str): The cron schedule string.
        base_date (datetime.datetime | None): The base date to start calculating from.
            If not provided, the current UTC date and time will be used.

    Returns:
        datetime.datetime: The next occurrence of the cron schedule (UTC).
    """
    crontab = parse_cron_string(cron_string)

    if base_date is None:
        base_date = datetime.datetime.now(datetime.timezone.utc)

    # Ensure base_date is timezone-aware (UTC)
    if base_date.tzinfo is None:
        base_date = base_date.replace(tzinfo=datetime.timezone.utc)

    # The calculation is done starting from the next minute
    base_date += datetime.timedelta(minutes=1)
    next_time = crontab.next_from(base_date)

    return next_time


def parse_cron_string(expression: str) -> Crontab:
    """
    Parse a crontab expression into a Crontab object.

    >>> parse_cron_string("*/10 * * * *")
    Crontab(...)
    """
