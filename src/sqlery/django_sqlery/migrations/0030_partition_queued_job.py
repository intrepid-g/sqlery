...
class _VendorGuardedCutover(migrations.RunPython):
    ...
    Forward (S1–S9):
      ...
      S2  CREATE TABLE sqlery_queued_job LIKE legacy PARTITION BY RANGE (created_at)
      S3  ADD PRIMARY KEY (created_at, id)
      S4  ADD GENERATED BY DEFAULT AS IDENTITY on id
      ...
      S9  Seed identity sequence past max(id)
    ...
    def _forward(_apps, schema_editor):
        ...
        with connection.cursor() as cursor:
            ...
            # S2 — Create partitioned table (NOT INCLUDING IDENTITY — senior finding #1)
            cursor.execute(
                """
            ...
            # S4 — Attach identity sequence to id column (catalog-guarded for idempotency)
            cursor.execute(
                """
            ...
            row = cursor.fetchone()
            # attidentity is '' (empty string) for regular columns, 'a' or 'd' for identity
            ...
            # S9 — Seed identity sequence past max(id) so new inserts continue correctly.
            # pg_get_serial_sequence works for GENERATED ... AS IDENTITY columns (PG >= 10).
            cursor.execute(
                "SELECT setval("
    ...
    def _backward(_apps, schema_editor):
        ...
        Creates an unpartitioned copy (LIKE sqlery_queued_job), copies all rows back,
        then renames the partitioned table out of the way and promotes the unpartitioned
        copy to sqlery_queued_job.  The legacy table (sqlery_queued_job_legacy) is NOT
        dropped — that is deferred per D3.
        """
        connection = schema_editor.connection
        with connection.cursor() as cursor:
            # Create unpartitioned rollback table
            cursor.execute(
                """
