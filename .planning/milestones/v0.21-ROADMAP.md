# Roadmap: Sqlery v0.21 — Feature-Complete Run Modes (ARCHIVED)

> **Milestone status:** ✅ Shipped 2026-05-15.
> 4 phases, 25 plans, 137 commits, 241 files changed (+29,598 / −5,086 LOC).
> 43/43 requirements verified. See `v0.21-REQUIREMENTS.md` and `.planning/v0.21-MILESTONE-AUDIT.md`.
>
> Key accomplishments:
> 1. **Core unified** — `import sqlery.core` works without Django installed; duplicate worker/claiming implementations retired to dated stubs.
> 2. **6 execution modes × 2 integrations** — daemon, subprocess, HTTP trigger, Lambda, async worker, synchronous — all E2E-tested across Django and standalone backends.
> 3. **Async rebuild from scratch** — AsyncWorker uses real async backends (psycopg3-async + aiosqlite) with drain-with-deadline shutdown.
> 4. **Test infrastructure** — `tests/unit/` scaffolded with FakeBackend; `tests/chaos/` rebuilt with real subprocess workers + Hypothesis; parametrized `(mode, integration, db)` E2E matrix; Postgres CI rail.
> 5. **Security hardening** — three-mode dashboard auth (default-on), webhook SSRF defense, CSRF protection on 10 admin endpoints, opt-in task module allowlist.
> 6. **Cleanup discipline** — 22 BC stubs date-stamped, dead code marked (never deleted), `worker_process.py:71` arity bug caught and fixed.
